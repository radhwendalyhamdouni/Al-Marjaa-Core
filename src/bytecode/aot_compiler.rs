...
use super::pgo::ProfilingManager;
use super::type_inference::{Type, TypeAnalysisResult, TypeInferenceEngine};
...
impl AotCompiler {
    ...
    pub fn compile_file(&mut self, path: &Path) -> Result<CompilationUnit, String> {
        let source =
            std::fs::read_to_string(path).map_err(|e| format!("خطأ في قراءة الملف: {}", e))?;
    ...
    fn compile_chunk(&mut self, chunk: &Chunk) -> Result<CompiledModule, String> {
        ...
        let compiled_instructions =
            self.compile_instructions(&chunk.instructions, &type_analysis)?;
        ...
        while i + 2 < func.code.len() {
            ...
            if let (
                Some(CompiledInstruction {
                    opcode: OpCode::PushNumber(a),
                    ..
                }),
                Some(CompiledInstruction {
                    opcode: OpCode::PushNumber(b),
                    ..
                }),
                Some(CompiledInstruction { opcode: op, .. }),
            ) = (func.code.get(i), func.code.get(i + 1), func.code.get(i + 2))
            {
                // حساب النتيجة
    ...
    fn calculate_memory_requirements(&mut self, module: &mut CompiledModule) {
        let code_size: usize = module
            .functions
            .values()
            .map(|f| f.code.len() * std::mem::size_of::<CompiledInstruction>())
            .sum();

        let stack_size: usize = module
            .functions
            .values()
            .map(|f| f.stack_size)
            .max()
    ...
    pub fn print_report(&self) {
        ...
        println!(
            "║    استنباط الأنواع: {:15}                                 ║",
            if self.settings.type_inference {
                "مفعّل"
            } else {
                "معطّل"
            }
...
mod tests {
    ...
    fn test_aot_compiler_basic() {
        ...
        let _result = compiler.compile_source(source, None);
    ...
    fn test_aot_settings() {
        ...
        assert_eq!(
            compiler.settings.optimization_level,
