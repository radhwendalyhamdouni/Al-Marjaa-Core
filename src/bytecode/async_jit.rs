...
impl AsyncStateMachine {
    ...
    pub fn advance(&mut self) -> Poll<AsyncResult> {
        ...
        if self.current_state < self.states.len() as u32 {
            ...
            if let Some(_await_point) = self.await_points.get(&self.current_state) {
...
impl AsyncJitCompiler {
    ...
    pub fn print_report(&self) {
        ...
        println!(
            "║    تخزين Wakers: {:15}                                        ║",
            if self.optimizations.waker_caching {
                "مفعّل"
            } else {
                "معطّل"
            }
...
impl AsyncRuntime {
    ...
    pub fn tick(&mut self) -> Option<AsyncResult> {
        if let Some(_task) = self.ready_queue.pop_front() {
            // إنشاء waker بسيط
