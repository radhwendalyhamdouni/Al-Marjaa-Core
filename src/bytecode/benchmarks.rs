...
use crate::bytecode::{run_bytecode, Compiler, ExecutionResult, VM};
...
pub fn print_benchmark_results(results: &[BenchmarkResult]) {
    ...
    println!(
        "║                        🚀 نتائج اختبارات Bytecode VM المحسّن                          ║"
    );
    println!(
        "╠══════════════════════════════════════════════════════════════════════════════════════╣"
    );
    println!(
        "║ الاختبار          │ الوقت(μs) │ التكرارات │ تعليمة/μs │ الكاش │ النتيجة            ║"
    );
    println!(
        "╠══════════════════════════════════════════════════════════════════════════════════════╣"
    );
    ...
    for r in results {
        ...
        println!(
            "║ {:17} │ {:9} │ {:9} │ {:9.2} │ {:4.0}% │ {:18} ║",
            r.name, r.vm_time_us, r.iterations, r.instructions_per_us, cache_ratio, r.result_value
        );
    }
...
fn benchmark_cache_efficiency() -> BenchmarkResult {
    let code = r#"
        ...
        طالما س < 500 {
            متغير نسخة = قيمة؛
            تراكم = تراكم + نسخة + قيمة + نسخة + قيمة؛
            قيمة = قيمة + 1؛
            س = س + 1؛
        }
    "#;
...
mod tests {
    ...
    #[test]
    #[cfg_attr(
        not(feature = "bench-heavy"),
        ignore = "اختبار benchmark ثقيل؛ شغّله عبر --features bench-heavy"
    )]
    ...
    fn test_all_benchmarks() {
        ...
        for r in &results {
            assert!(
                r.vm_time_us > 0,
                "Benchmark {} should have positive time",
                r.name
            );
        }
    }
