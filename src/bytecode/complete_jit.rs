...
impl JitStats {
    pub fn total_executions(&self) -> u64 {
        self.tier0_executions
            + self.tier1_executions
            + self.tier2_executions
            + self.tier3_executions
            + self.tier4_executions
    }
    ...
    pub fn avg_speedup(&self) -> f64 {
        ...
        let tiered = self.tier1_executions
            + self.tier2_executions
            + self.tier3_executions
            + self.tier4_executions;
...
impl CompleteJitCompiler {
    ...
    pub fn record_execution(&mut self, ip: usize) -> bool {
        ...
        new_tier > current_tier && !is_compiled
    }
    ...
    fn compile_tier0(&mut self, chunk: &Chunk, _start_ip: usize) -> Result<CompiledCode, String> {
        let instructions: Vec<CompiledInstruction> = chunk
            .instructions
            .iter()
        ...
        while i < code.instructions.len() {
            ...
            if i + 2 < code.instructions.len() {
                if let (
                    ...
                    if let (Some(Value::Number(n1)), Some(Value::Number(n2))) =
                        (code.constants.get(*idx1), code.constants.get(*idx2))
                    {
    ...
    fn fold_binary_op(&self, op: &OpCode, a: f64, b: f64) -> Option<f64> {
        match op {
            ...
            OpCode::Equal => Some(if (a - b).abs() < f64::EPSILON {
                1.0
            } else {
                0.0
            }),
            OpCode::NotEqual => Some(if (a - b).abs() >= f64::EPSILON {
                1.0
            } else {
                0.0
            }),
    ...
    fn detect_simd_pattern(
        ...
        for instr in instructions
            .iter()
            .skip(start)
            .take(instructions.len().min(start + count * 2) - start)
        {
            if matches!(instr, CompiledInstruction::PushConst(_)) {
                num_count += 1;
            }
        }
        num_count >= count
    ...
    fn add_type_guards(&mut self, code: &mut CompiledCode) {
        ...
        for (i, instr) in code.instructions.iter().enumerate() {
            if let CompiledInstruction::LoadLocalFast { slot: _, .. } = instr {
                guards.push(Guard {
                    guard_type: GuardType::TypeCheck {
                        expected: TypeInfo::Number,
                    },
                    location: i,
    ...
    fn optimize_hot_paths(&mut self, code: &mut CompiledCode) {
        ...
        for _start in loop_starts.keys() {
    ...
    pub fn execute(
        &mut self,
        chunk: &Chunk,
        globals: &mut Rc<RefCell<Environment>>,
    ) -> Result<Value, String> {
        let start_ip = 0;
    ...
    fn execute_tier4(
        ...
        while ip < compiled.instructions.len() {
            ...
            for guard in &compiled.guards {
                if guard.location == ip && !self.check_guard(guard)? {
        ...
        match op {
            ...
            OpCode::PushString(idx) => {
                ...
                self.execution_stack
                    .push(Value::String(format!("str_{}", idx)));
            }
            OpCode::PushBool(b) => {
            ...
            OpCode::And => {
                ...
                self.execution_stack
                    .push(Value::Boolean(self.is_truthy(&a) && self.is_truthy(&b)));
            }
            OpCode::Or => {
                let b = self.execution_stack.pop().unwrap_or(Value::Null);
                let a = self.execution_stack.pop().unwrap_or(Value::Null);
            ...
            OpCode::Not => {
                if let Some(val) = self.execution_stack.pop() {
                    self.execution_stack
                        .push(Value::Boolean(!self.is_truthy(&val)));
                }
            }
            OpCode::Print => {
                if let Some(val) = self.execution_stack.pop() {
    ...
    fn execute_simd_op(&mut self, op: &SimdOp, count: usize) -> Result<(), String> {
        match op {
            SimdOp::AddF64x4 => {
                ...
                results
                    .into_iter()
                    .for_each(|r| self.execution_stack.push(r));
            }
            SimdOp::MulF64x4 => {
                let mut results = Vec::with_capacity(count);
                for _ in 0..count {
    ...
    pub fn print_report(&self) {
        ...
        println!(
            "║           🚀 تقرير JIT Compiler الكامل - لغة المرجع                          ║"
        );
        println!(
            "╠══════════════════════════════════════════════════════════════════════════════╣"
        );
        ...
        println!(
            "║    Tier 0 (Interpreter): {:12} تنفيذ                               ║",
            self.stats.tier0_executions
        );
        println!(
            "║    Tier 1 (Baseline):    {:12} تنفيذ                               ║",
            self.stats.tier1_executions
        );
        println!(
            "║    Tier 2 (Optimizing):  {:12} تنفيذ                               ║",
            self.stats.tier2_executions
        );
        println!(
            "║    Tier 3 (SIMD):        {:12} تنفيذ                               ║",
            self.stats.tier3_executions
        );
        println!(
            "║    Tier 4 (Tracing):     {:12} تنفيذ                               ║",
            self.stats.tier4_executions
        );
        ...
        println!(
            "║    وقت التجميع: {} μs                                                   ║",
            self.stats.total_compile_time_us
        );
        println!(
