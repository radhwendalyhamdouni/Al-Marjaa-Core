...
impl GcStats {
    ...
    pub fn update_ratios(&mut self) {
        if self.total_collections > 0 {
            self.cache_hit_ratio = 1.0
                - (self.objects_collected as f64
                    / (self.objects_collected + self.objects_surviving) as f64)
                    .max(0.0);
        }
    }
}
...
impl ParallelGc {
    ...
    fn estimate_size(value: &Value) -> usize {
        match value {
            ...
            Value::Dictionary(d) => {
                d.len() * (std::mem::size_of::<String>() + std::mem::size_of::<SharedValue>())
            }
            ...
            Value::Tensor { data, shape } => {
                data.len() * std::mem::size_of::<f64>() + shape.len() * std::mem::size_of::<usize>()
            }
            Value::AutoTensor { data, grad, .. } => {
                (data.len() + grad.len()) * std::mem::size_of::<f64>()
            }
            _ => 64, // تقدير افتراضي
        }
    }
    ...
    pub fn collect_old(&self) {
        ...
        // مرحلة المسح للجيلين
        let sweep_start = Instant::now();
        let (young_collected, young_promoted, young_freed) =
            self.sweep_phase_parallel(Generation::Young);
    ...
    fn mark_phase_parallel(&self, generation: Generation) {
        ...
        let initial_work: Vec<GcObjectId> = roots
            .iter()
            .filter_map(|root| {
    ...
    fn sweep_phase_parallel(&self, generation: Generation) -> (u64, u64, u64) {
        ...
        match generation {
            Generation::Young => {
                ...
                // تقسيم للمعالجة
                let to_remove: Vec<GcObjectId> = young
                    .iter()
                    .filter(|obj| !obj.marked.get())
                    .map(|obj| obj.id)
                    .collect();

                let to_promote: Vec<GcObjectId> = young
                    .iter()
            ...
            Generation::Old => {
                ...
                let to_remove: Vec<GcObjectId> = old
                    .iter()
                    .filter(|obj| !obj.marked.get())
                    .map(|obj| obj.id)
                    .collect();
        ...
        {
            let mut stats = self.stats.write().unwrap();
            stats.objects_surviving +=
                (self.young_gen.read().unwrap().len() + self.old_gen.read().unwrap().len()) as u64;
            stats.bytes_used = (self
                .young_gen
                .read()
                .unwrap()
                .iter()
    ...
    pub fn print_report(&self) {
        ...
        println!(
            "║   ├── دورات الجيل الشاب: {:>10}                         ║",
            stats.young_collections
        );
        println!(
            "║   └── دورات الجيل القديم: {:>10}                        ║",
            stats.old_collections
        );
        println!("╠══════════════════════════════════════════════════════════════╣");
        println!("║ ⏱️ الأداء:                                                   ║");
        println!(
            "║   ├── إجمالي الوقت: {:>10} مللي ثانية                    ║",
            stats.total_time_ms
        );
        println!(
            "║   ├── وقت العلام: {:>10} مللي ثانية                      ║",
            stats.mark_time_ms
        );
        println!(
            "║   └── وقت المسح: {:>10} مللي ثانية                       ║",
            stats.sweep_time_ms
        );
        println!("╠══════════════════════════════════════════════════════════════╣");
        println!("║ 🗑️ الكائنات:                                                 ║");
        println!(
            "║   ├── الكائنات المجموعة: {:>10}                          ║",
            stats.objects_collected
        );
        println!(
            "║   ├── الكائنات المتبقية: {:>10}                          ║",
            stats.objects_surviving
        );
        println!(
            "║   └── الكائنات المرقاة: {:>10}                           ║",
        ...
        println!("║ 💾 الذاكرة:                                                  ║");
        println!(
            "║   ├── الذاكرة المستخدمة: {:>10} بايت                     ║",
            stats.bytes_used
        );
        println!(
            "║   ├── الذاكرة المحررة: {:>10} بايت                       ║",
            stats.bytes_freed
        );
        println!(
            "║   ├── كائنات الجيل الشاب: {:>10}                         ║",
            young
        );
        println!(
            "║   └── كائنات الجيل القديم: {:>10}                        ║",
            old
        );
        println!("╠══════════════════════════════════════════════════════════════╣");
        println!("║ 🔧 التوازي:                                                  ║");
...
mod tests {
    ...
    fn test_memory_manager() {
        ...
        let _v1 = manager.alloc(Value::Number(10.0));
