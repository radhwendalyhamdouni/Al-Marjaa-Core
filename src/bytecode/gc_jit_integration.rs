...
use super::gc::{Generation, ParallelGc};
...
impl SafepointManager {
    ...
    pub fn add_safepoint(&mut self, safepoint: Safepoint) {
        self.safepoints
            .insert(safepoint.instruction_offset, safepoint);
    }
    ...
    pub fn needs_safepoint(&self, ip: usize) -> bool {
        ...
        ip.is_multiple_of(self.default_frequency)
    }
...
pub enum WriteLocation {
    /// حقل في كائن
    ObjectField { object_reg: u8, field_offset: usize },
    /// عنصر في مصفوفة
    ArrayElement { array_reg: u8, index_reg: u8 },
    /// متغير عام
    Global { name: String },
    /// متغير محلي
    Local { slot: u32 },
...
impl AllocationManager {
    ...
    pub fn new() -> Self {
        AllocationManager {
            ...
            strategy: AllocationStrategy::SizeBased {
                young_threshold: 256,
            },
        }
    }
    ...
    pub fn record_allocation(&mut self, site: usize, size: usize) -> Generation {
        ...
        self.stats.avg_allocation_size =
            self.stats.total_bytes as f64 / self.stats.total_allocations as f64;
...
impl GcJitCoordinator {
    ...
    pub fn execute_write_barrier(
        ...
        let barrier_type = self
            .write_barrier_generator
            .generate_barrier(location, source_gen, target_gen);

        if barrier_type != WriteBarrierType::None {
            self.stats.barriers_executed += 1;

            // تسجيل المرجع في الـ remembered set
            if let Some(ref _gc) = self.gc {
    ...
    pub fn allocate_in_jit(&mut self, site: usize, size: usize) -> Option<SharedValue> {
        ...
        let _generation = self.allocation_manager.record_allocation(site, size);
    ...
    pub fn print_report(&self) {
        ...
        println!(
            "║    نقاط الأمان المستخدمة: {:15}                              ║",
            self.stats.safepoint_hits
        );
        println!(
            "║    مجموعات GC المطلوبة: {:15}                                ║",
            self.stats.gc_collections_triggered
        );
        println!(
            "║    حواجز الكتابة المنفذة: {:15}                             ║",
            self.stats.barriers_executed
        );
        println!(
            "║    التخصيصات في JIT: {:15}                                   ║",
            self.stats.allocations_in_jit_code
        );
        ...
        println!(
            "║    وقت GC: {} ns                                                      ║",
            self.stats.time_in_gc_ns
        );
        println!(
            "║    وقت JIT: {} ns                                                     ║",
            self.stats.time_in_jit_ns
        );
        ...
        println!(
            "║    إجمالي البايتات: {:15}                                     ║",
            alloc_stats.total_bytes
        );
        println!(
            "║    متوسط الحجم: {:.2} bytes                                           ║",
            alloc_stats.avg_allocation_size
        );
