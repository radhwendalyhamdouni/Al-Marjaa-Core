...
impl JitCompiler {
    ...
    pub fn print_report(&self) {
        ...
        println!(
            "║ تنفيذ JIT:           {:15}                     ║",
            self.stats.jit_executions
        );
        println!(
            "║ تنفيذ بطيء:          {:15}                     ║",
            self.stats.slow_path_executions
        );
        println!(
...
impl OptimizedExecutor {
    ...
    pub fn execute(
        ...
        match code {
            CompiledCode::OptimizedBytecode {
                instructions,
                constants,
            } => self.execute_optimized(instructions, constants, globals),
            CompiledCode::Bytecode(opcodes) => self.execute_bytecode(opcodes, globals),
            CompiledCode::Trace {
                entries,
                exit_points,
            ...
            match op {
                ...
                OptimizedOp::StoreLocalFast(slot) => {
                    if let Some(val) = self.stack.pop() {
                        ...
                        if slot >= self.locals.len() {
                            self.locals
                                .resize(slot + 1, Rc::new(RefCell::new(Value::Null)));
                        }
                ...
                OptimizedOp::Native(native_op) => {
                    ...
                    match native_op {
                        ...
                        OpCode::Div => {
                            self.binary_op(|a, b| if b == 0.0 { f64::INFINITY } else { a / b })?
                        }
                        OpCode::Halt => break,
                        _ => {}
                    }
                ...
                OptimizedOp::BinaryOpConst { op, const_idx } => {
                    if let Some(&Value::Number(c)) = constants.get(*const_idx) {
                ...
                OptimizedOp::FastLoop {
                    ...
                    for i in *start..*end {
                        self.locals
                            .push(Rc::new(RefCell::new(Value::Number(i as f64))));
                        // تنفيذ جسم الحلقة
