...
impl Optimizer {
    ...
    fn optimize_stmt(&mut self, stmt: &Stmt) -> Option<Stmt> {
        match stmt {
            ...
            Stmt::Print(exprs) => {
                let optimized: Vec<Expr> = exprs.iter().map(|e| self.optimize_expr(e)).collect();
            ...
            Stmt::If {
                ...
                let optimized_elif: Vec<(Expr, Stmt)> = else_if_branches
                    .iter()
                    .filter_map(|(c, b)| {
                ...
                Some(Stmt::If {
                    ...
                    else_if_branches: optimized_elif
                        .into_iter()
                        .map(|(c, b)| (c, Box::new(b)))
                        .collect(),
            ...
            Stmt::Block(statements) => {
                let optimized: Vec<Stmt> = statements
                    .iter()
    ...
    fn optimize_expr(&mut self, expr: &Expr) -> Expr {
        match expr {
            ...
            Expr::Call { callee, args } => {
                ...
                let opt_args: Vec<Expr> = args.iter().map(|a| self.optimize_expr(a)).collect();
            ...
            Expr::List(elements) => {
                let optimized: Vec<Expr> = elements.iter().map(|e| self.optimize_expr(e)).collect();
                Expr::List(optimized)
            }

            Expr::Dictionary(entries) => {
                let optimized: Vec<(Expr, Expr)> = entries
                    .iter()
                ...
                if let (Expr::List(elements), Expr::Number(idx)) = (&opt_object, &opt_index) {
                    ...
                    if i < elements.len() && self.is_pure_expr(&elements[i]) {
                        self.add_detail(
                            OptimizationKind::ConstantFolding,
                            format!("list[{}]", idx),
                            "تبسيط فهرسة قائمة ثابتة".to_string(),
                        );
