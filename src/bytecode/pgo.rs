...
impl ProfilingManager {
        ...
        if let Some(profile) = self.loop_profiles.get_mut(&start_address) {
            if profile.entry_count > 0 {
                profile.avg_iterations =
                    profile.total_iterations as f64 / profile.entry_count as f64;
            }
        }
    }
    ...
    fn calculate_hotness(&mut self) {
        ...
        for profile in self.loop_profiles.values_mut() {
            profile.time_percentage = (profile.total_iterations as f64
                / self.global_stats.total_loop_iterations.max(1) as f64)
                * 100.0;
    ...
    pub fn get_hot_loops(&self) -> Vec<&LoopProfile> {
        self.loop_profiles.values().filter(|p| p.is_hot).collect()
    }
    ...
    pub fn get_common_types(&self) -> HashMap<usize, String> {
        self.instruction_profiles
            .iter()
            .filter_map(|(ip, profile)| profile.most_common_type().map(|(t, _)| (*ip, t)))
            .collect()
    }
    ...
    pub fn save_to_file(&self, path: &Path) -> std::io::Result<()> {
        ...
        writeln!(
            ...
            self.global_stats.total_execution_time_ns
        )?;
        writeln!(
            writer,
            "total_instructions={}",
            self.global_stats.total_instructions
        )?;
        writeln!(
            writer,
            "total_function_calls={}",
            self.global_stats.total_function_calls
        )?;
        writeln!(
            writer,
            "total_branches={}",
            self.global_stats.total_branches
        )?;
        writeln!(
            writer,
            "total_loop_iterations={}",
            self.global_stats.total_loop_iterations
        )?;
    ...
    pub fn print_report(&self) {
        ...
        println!(
            "║    الدوال الساخنة: {:15}                                        ║",
            self.global_stats.hot_functions_count
        );
        println!(
            "║    الحلقات الساخنة: {:15}                                      ║",
            self.global_stats.hot_loops_count
        );

        println!("╠══════════════════════════════════════════════════════════════════════════╣");
        println!("║ 🔥 الدوال الساخنة:                                                        ║");

        let mut hot_functions: Vec<_> = self
            .function_profiles
            .values()
            .filter(|p| p.is_hot)
            .collect();
        ...
        for profile in hot_functions.iter().take(5) {
            println!(
                "║    {}() - {} استدعاء، {:.2}٪ من الوقت              ║",
                profile.name, profile.call_count, profile.hotness_score
            );
        }
        ...
        let mut hot_loops: Vec<_> = self.loop_profiles.values().filter(|p| p.is_hot).collect();
        hot_loops.sort_by(|a, b| b.total_iterations.cmp(&a.total_iterations));

        for profile in hot_loops.iter().take(5) {
            println!(
                "║    @{} - {} تكرار، متوسط {:.1}                           ║",
                profile.start_address, profile.total_iterations, profile.avg_iterations
            );
        }
...
impl PgoOptimizer {
    ...
    pub fn analyze(&mut self) {
        ...
        for func in profiling.get_hot_functions() {
            ...
            if func.call_count > 100 && func.avg_time_ns < 1000.0 {
                ...
                self.optimization_decisions
                    .push(OptimizationDecision::InlineFunction {
                        function_name: func.name.clone(),
                        call_site: func.start_address,
                        benefit_score: func.call_count as f64 * (1000.0 - func.avg_time_ns)
                            / 1000.0,
                    });
        ...
        for (addr, profile) in &profiling.branch_profiles {
            if profile.taken_rate > 0.95 || profile.taken_rate < 0.05 {
                // تفرع شبه ثابت
                self.optimization_decisions
                    .push(OptimizationDecision::OptimizeBranch {
                        address: *addr,
        ...
        for profile in profiling.get_hot_loops() {
            ...
            self.optimization_decisions
                .push(OptimizationDecision::OptimizeLoop {
        ...
        for (addr, value) in profiling.get_constant_values() {
            self.optimization_decisions
                .push(OptimizationDecision::SpecializeValue {
                    address: addr,
        ...
        for (addr, type_name) in profiling.get_common_types() {
            self.optimization_decisions
                .push(OptimizationDecision::SpecializeType {
                    address: addr,
        ...
        self.optimization_stats.estimated_speedup = 1.0
            + (self.optimization_stats.functions_inlined as f64 * 0.1)
    ...
    pub fn print_report(&self) {
        ...
        println!(
            "║    تفرعات محسّنة: {:15}                                   ║",
            self.optimization_stats.branches_optimized
        );
        println!(
            "║    حلقات محسّنة: {:15}                                      ║",
            self.optimization_stats.loops_optimized
        );
        println!(
            "║    قيم متخصصة: {:15}                                      ║",
            self.optimization_stats.values_specialized
        );
        println!(
            "║    أنواع متخصصة: {:15}                                    ║",
            self.optimization_stats.types_specialized
        );
        println!(
            "║    السرعة المتوقعة: {:.2}x                                             ║",
        ...
        for decision in self.optimization_decisions.iter().take(10) {
            match decision {
                ...
                OptimizationDecision::OptimizeBranch {
                    address,
                    predicted_taken,
                    ..
                } => {
                ...
                OptimizationDecision::OptimizeLoop {
                    start_address,
                    unroll_factor,
                    ..
                } => {
                ...
                OptimizationDecision::SpecializeValue {
                    address,
                    constant_value,
                    ..
                } => {
                ...
                OptimizationDecision::SpecializeType {
                    address,
                    expected_type,
                    ..
                } => {
