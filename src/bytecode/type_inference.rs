...
impl Type {
    ...
    pub fn can_implicitly_cast_to(&self, target: &Type) -> bool {
        ...
        match (self, target) {
            ...
            (Type::Union(types), target) => types.iter().any(|t| t.can_implicitly_cast_to(target)),
            (t, Type::Union(types)) => types.iter().any(|tt| t.can_implicitly_cast_to(tt)),
    ...
    pub fn merge(&self, other: &Type) -> Type {
        ...
        match (self, other) {
            ...
            (Type::Optional(a), Type::Optional(b)) => Type::optional(a.merge(b)),
    ...
    pub fn to_string_repr(&self) -> String {
        match self {
            ...
            Type::Function(params, ret) => {
                let params_str: Vec<String> = params.iter().map(|p| p.to_string_repr()).collect();
                format!(
                    "دالة({}) -> {}",
                    params_str.join(", "),
                    ret.to_string_repr()
            ...
            Type::Union(types) => {
                let types_str: Vec<String> = types.iter().map(|t| t.to_string_repr()).collect();
...
impl TypeInferenceEngine {
    ...
    fn analyze_opcode(&mut self, ip: usize, opcode: &OpCode) {
        match opcode {
            ...
            OpCode::BuildDict(count) => {
                ...
                self.type_stack
                    .push(Type::dict_of(key_type.clone(), value_type.clone()));
                self.location_types
                    .insert(ip, Type::dict_of(key_type, value_type));
            }
            ...
            OpCode::JumpIfFalse(_) | OpCode::JumpIfTrue(_) => {
                ...
                if let Some(cond_type) = self.type_stack.pop() {
                    if cond_type != Type::Boolean && cond_type != Type::Unknown {
                        self.warnings.push(TypeWarning {
                            message: format!("شرط غير منطقي: {}", cond_type.to_string_repr()),
                            location: ip,
                        });
                    }
                }
    ...
    fn analyze_unary_numeric_op(&mut self, ip: usize) {
        ...
        match a_type {
            ...
            Some(other) => {
                self.errors.push(TypeError {
                    message: format!("عملية حسابية على نوع غير رقمي: {}", other.to_string_repr()),
                    location: ip,
                    expected: Type::Number,
                    actual: other,
                });
    ...
    fn generate_guards(&mut self) {
        ...
        for var_info in self.symbol_table.values() {
    ...
    pub fn print_report(&self) {
        ...
        for guard in &self.guards {
            println!(
                "║    [{}] {:?} في {}                                     ║",
                guard.id, guard.guard_type, guard.location
            );
        }
...
mod tests {
    ...
    fn test_type_inference_list() {
        ...
        assert!(result.errors.is_empty());
        assert_eq!(
            result.location_types.get(&3),
            Some(&Type::list_of(Type::Number))
        );
    }
    ...
    fn test_guard_optimization() {
        let mut guard = TypeGuard::new(1, TypeGuardKind::TypeCheck, 0, Type::Number);
