...
impl VM {
        ...
        loop {
            match self.step() {
                ...
                ExecutionResult::Break => {
                    return ExecutionResult::Error("break خارج الحلقة".into())
                }
                ExecutionResult::Continue => {
                    return ExecutionResult::Error("continue خارج الحلقة".into())
                }
    ...
    fn execute_opcode(&mut self, opcode: &OpCode) -> ExecutionResult {
        match opcode {
            ...
            OpCode::Div => self.binary_op_fast(|a, b| if b == 0.0 { f64::INFINITY } else { a / b }),
            ...
            OpCode::StoreGlobal(idx) => {
                ...
                if let Some(name) = name {
                    ...
                    self.globals
                        .borrow_mut()
                        .define(&name, (*value.borrow()).clone(), false);
                    // تحديث الكاش
            ...
            OpCode::StoreLocal(slot) => {
                ...
                if index < self.stack.len() {
                    ...
                    // عندما يكون المتغير المحلي في قمة المكدس، pop يقلّص الطول
                    // لذا نعيد وضع القيمة في نفس الفتحة المنطقية.
            ...
            OpCode::DefineConst(idx) => {
                ...
                if let Some(name) = name {
                    ...
                    self.globals
                        .borrow_mut()
                        .define(&name, (*value.borrow()).clone(), true);
                }
                ExecutionResult::Ok(Rc::new(RefCell::new(Value::Null)))
            }
                ...
                match (&obj, &index) {
                    ...
                    (Value::String(s), Value::Number(idx)) => {
                        match self.normalize_index(*idx, s.len()) {
                            Ok(i) => self
                                .push_value(Value::String(s.chars().nth(i).unwrap().to_string())),
                            Err(e) => return e,
                        }
                    }
            ...
            // الحلقات
            // ═══════════════════════════════════════════════════════════════
            OpCode::LoopStart(_slot) => ExecutionResult::Ok(Rc::new(RefCell::new(Value::Null))),

            OpCode::LoopNext(_offset) => ExecutionResult::Ok(Rc::new(RefCell::new(Value::Null))),

            OpCode::LoopEnd => ExecutionResult::Ok(Rc::new(RefCell::new(Value::Null))),
            ...
            OpCode::TryStart(_offset) => ExecutionResult::Ok(Rc::new(RefCell::new(Value::Null))),

            OpCode::TryEnd => ExecutionResult::Ok(Rc::new(RefCell::new(Value::Null))),
            ...
            OpCode::Nop => ExecutionResult::Ok(Rc::new(RefCell::new(Value::Null))),
            ...
            OpCode::AsyncReturn => ExecutionResult::Return(Rc::new(RefCell::new(Value::Null))),
