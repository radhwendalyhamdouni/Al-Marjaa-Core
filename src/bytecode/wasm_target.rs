...
impl WasmCompiler {
    ...
    fn compile_opcode(&mut self, opcode: &OpCode, func: &mut WasmFunction) -> Result<(), String> {
        match opcode {
            OpCode::PushNumber(n) => {
                func.instructions
                    .push(WasmInstruction::F64Const { value: *n });
            }

            OpCode::PushBool(b) => {
                func.instructions.push(WasmInstruction::I32Const {
                    value: if *b { 1 } else { 0 },
                });
            }
            ...
            OpCode::PushNull => {
                // NULL كـ 0
                func.instructions
                    .push(WasmInstruction::I32Const { value: 0 });
            }
            ...
            OpCode::Mod => {
                ...
                func.instructions
                    .push(WasmInstruction::Call { func_idx: 0 }); // دالة mod
            }

            OpCode::Pow => {
                // استدعاء دالة pow
                func.instructions
                    .push(WasmInstruction::Call { func_idx: 1 });
            }

            OpCode::Neg => {
                func.instructions
                    .push(WasmInstruction::F64Const { value: -1.0 });
                func.instructions.push(WasmInstruction::F64Mul);
            }
            ...
            OpCode::Jump(offset) => {
                ...
                func.instructions.push(WasmInstruction::Br {
                    depth: *offset as u32,
                });
            }
            ...
            OpCode::JumpIfFalse(offset) => {
                ...
                func.instructions.push(WasmInstruction::BrIf {
                    depth: *offset as u32,
                });
            }
    ...
    fn emit_export_section(&self, buffer: &mut Vec<u8>, module: &WasmModule) -> Result<(), String> {
        let exports: Vec<(String, u8, u32)> = module
            .functions
            .iter()
            .enumerate()
    ...
    pub fn print_report(&self) {
        ...
        println!(
            "║    الذاكرة المستخدمة: {} bytes                                        ║",
            self.stats.memory_used
        );
        println!(
            "║    وقت الترجمة: {} μs                                                ║",
            self.stats.compile_time_us
        );
