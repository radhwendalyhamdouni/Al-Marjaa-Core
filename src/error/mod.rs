...
impl AlMarjaaError {
    ...
    /// Format error for display with colors
    pub fn format(&self, filename: &str) -> String {
        ...
        let severity_colored = match self.severity {
            Severity::Warning => {
                format!("{} {}", self.severity.icon(), self.severity.arabic_name())
                    .bright_yellow()
                    .bold()
                    .to_string()
            }
            Severity::Error => format!("{} {}", self.severity.icon(), self.severity.arabic_name())
                .bright_red()
                .bold()
                .to_string(),
            Severity::Critical => {
        ...
        if let Some(span) = &self.span {
            output.push_str(&format!(
                ...
                span.start.line.to_string().bright_yellow(),
                span.start.column.to_string().bright_yellow()
            ));
            ...
            if let Some(ctx) = &self.source_context {
                ...
                output.push_str(&format!(
                    "{} {} {}\n",
                    span.start.line.to_string().bright_blue(),
                    "|".bright_blue(),
                    ctx
                ));
                // Caret pointer under the error
                let spaces = span.start.column.saturating_sub(1);
                let underline_len = if span.end.column > span.start.column + 1 {
        ...
        output.push_str(&format!(
            " {} {}\n",
            "|".bright_blue(),
            self.code.arabic_message().dimmed()
        ));

        // Suggestion with hint
        if let Some(suggestion) = &self.suggestion {
            output.push_str(&format!(
                " {} {} {}؟\n",
                "=".bright_cyan(),
                "هل تقصد:".bright_green(),
                suggestion.bright_green().italic()
            ));
        }

        // Help messages
        for help in &self.help {
            output.push_str(&format!(" {} {}\n", "=".bright_cyan(), help.bright_cyan()));
        }

        output
    }

    /// Format error for display without colors (for logging/files)
    pub fn format_plain(&self, filename: &str) -> String {
        let mut output = String::new();
