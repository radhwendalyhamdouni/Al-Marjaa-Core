...
pub fn define_network_funcs(env: &mut Environment) {
    ...
    fn execute_http_request(
        ...
        // تنفيذ الطلب
        let response = request.send().map_err(|e| format!("خطأ في الطلب: {}", e))?;
    ...
    fn execute_http_request_fallback(
        ...
        if lines.len() >= 2 {
            let status: u16 = lines.last().unwrap_or(&"0").parse().unwrap_or(0);
    ...
    fn http_response_to_value(text: String, status: u16) -> SharedValue {
        ...
        dict.insert("نص".to_string(), Rc::new(RefCell::new(Value::String(text))));
        dict.insert(
            "رمز".to_string(),
