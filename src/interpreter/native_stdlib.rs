            ...
            func: |a| match &*a[0].borrow() {
                Value::String(s) => {
                    if let Value::String(sub) = &*a[1].borrow() {
                        ...
                        Err("ابحث في نص يتطلب نصاً فرعياً".to_string())
                    }
                }
                Value::List(l) => {
...
pub fn define_ai_funcs(env: &mut Environment) {
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                let x = a[0].borrow().to_number()?;
                let alpha = if a.len() > 1 {
                    a[1].borrow().to_number()?
                } else {
                    0.01
                };
                Ok(Rc::new(RefCell::new(Value::Number(if x > 0.0 {
                    x
                } else {
                    alpha * x
                }))))
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                info.insert(
                    "طبقات".to_string(),
                    Rc::new(RefCell::new(Value::Number(layers_count as f64))),
                );
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                let updated: Vec<SharedValue> = weights
                    .iter()
                    .zip(gradients.iter())
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                info.insert(
                    "طريقة".to_string(),
                    Rc::new(RefCell::new(Value::String("GET".to_string()))),
                );
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                if trimmed.starts_with('{') && trimmed.ends_with('}') {
                    ...
                    dict.insert(
                        "نوع".to_string(),
                        Rc::new(RefCell::new(Value::String("كائن".to_string()))),
                    );
                    dict.insert(
                        "أصلي".to_string(),
                        Rc::new(RefCell::new(Value::String(json_str))),
                    );
                    return Ok(Rc::new(RefCell::new(Value::Dictionary(dict))));
                } else if trimmed.starts_with('[') && trimmed.ends_with(']') {
                    // مصفوفة JSON
                    return Ok(Rc::new(RefCell::new(Value::List(vec![Rc::new(
                        RefCell::new(Value::String("مصفوفة_جسون".to_string())),
                    )]))));
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                result.insert(
                    "وزن".to_string(),
                    Rc::new(RefCell::new(Value::Number(new_weight))),
                );
                result.insert(
                    "سرعة".to_string(),
                    Rc::new(RefCell::new(Value::Number(new_velocity))),
                );
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                let beta1 = if a.len() > 3 {
                    a[3].borrow().to_number()?
                } else {
                    0.9
                };
                let beta2 = if a.len() > 4 {
                    a[4].borrow().to_number()?
                } else {
                    0.999
                };
                let m = if a.len() > 5 {
                    a[5].borrow().to_number()?
                } else {
                    0.0
                };
                let v = if a.len() > 6 {
                    a[6].borrow().to_number()?
                } else {
                    0.0
                };
                let t = if a.len() > 7 {
                    a[7].borrow().to_number()? as i64
                } else {
                    1
                };
                ...
                result.insert(
                    "زمن".to_string(),
                    Rc::new(RefCell::new(Value::Number((t + 1) as f64))),
                );
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                let gamma = if a.len() > 3 {
                    a[3].borrow().to_number()?
                } else {
                    0.9
                };
                let cache = if a.len() > 4 {
                    a[4].borrow().to_number()?
                } else {
                    0.0
                };
                ...
                result.insert(
                    "مخزن".to_string(),
                    Rc::new(RefCell::new(Value::Number(new_cache))),
                );
...
pub fn define_training_funcs(env: &mut Environment) {
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                result.insert(
                    "معدل_تعلم".to_string(),
                    Rc::new(RefCell::new(Value::Number(lr))),
                );
                result.insert(
                    "خسارة".to_string(),
                ...
                if let Value::NeuralNetwork { name, layers } = model {
                    result.insert(
                        "نموذج".to_string(),
                        Rc::new(RefCell::new(Value::String(name))),
                    );
                    result.insert(
                        "طبقات".to_string(),
                        Rc::new(RefCell::new(Value::Number(layers.len() as f64))),
                    );
                }
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                let factor = if a.len() > 1 {
                    a[1].borrow().to_number()?
                } else {
                    0.5
                };
                let patience = if a.len() > 2 {
                    a[2].borrow().to_number()? as i64
                } else {
                    5
                };
                let best_loss = if a.len() > 3 {
                    a[3].borrow().to_number()?
                } else {
                    f64::INFINITY
                };
                let current_loss = if a.len() > 4 {
                    a[4].borrow().to_number()?
                } else {
                    f64::INFINITY
                };
                let wait = if a.len() > 5 {
                    a[5].borrow().to_number()? as i64
                } else {
                    0
                };
                ...
                result.insert(
                    "معدل_تعلم".to_string(),
                    Rc::new(RefCell::new(Value::Number(new_lr))),
                );
                result.insert(
                    "انتظار".to_string(),
                    Rc::new(RefCell::new(Value::Number(new_wait as f64))),
                );
                result.insert(
                    "أفضل_خسارة".to_string(),
                    Rc::new(RefCell::new(Value::Number(new_best))),
                );
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                result.insert(
                    "توقف".to_string(),
                    Rc::new(RefCell::new(Value::Boolean(should_stop))),
                );
                result.insert(
                    "انتظار".to_string(),
                    Rc::new(RefCell::new(Value::Number(new_wait as f64))),
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                let weights = match &*a[1].borrow() {
                    Value::List(l) => l
                        .iter()
                        .map(|v| v.borrow().to_number().unwrap_or(0.0))
                        .collect::<Vec<_>>(),
                    Value::Tensor { data, .. } => data.clone(),
                ...
                let gradients: Vec<SharedValue> = weights
                    .iter()
                    .map(|_| {
                        Rc::new(RefCell::new(Value::Number(
                            loss_grad / weights.len() as f64,
                        )))
                    })
                    .collect();
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                let input_size = a[0].borrow().to_number()? as usize;
                let output_size = if a.len() > 1 {
                    a[1].borrow().to_number()? as usize
                } else {
                    input_size
                };
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                module.insert(
                    "صادرات".to_string(),
                    Rc::new(RefCell::new(Value::List(Vec::new()))),
                );
                module.insert(
                    "واردات".to_string(),
                    Rc::new(RefCell::new(Value::List(Vec::new()))),
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                export_entry.insert(
                    "اسم".to_string(),
                    Rc::new(RefCell::new(Value::String(export_name))),
                );
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                module.insert(
                    "حالة".to_string(),
                    Rc::new(RefCell::new(Value::String("محمّلة".to_string()))),
                );
                module.insert(
                    "صادرات".to_string(),
                    Rc::new(RefCell::new(Value::List(Vec::new()))),
            ...
            func: |a| {
                ...
                let model_data = match &*model.borrow() {
                    Value::NeuralNetwork { name, layers } => {
                        ...
                        data.insert(
                            "نوع".to_string(),
                            Rc::new(RefCell::new(Value::String("شبكة_عصبية".to_string()))),
                        );
                        data.insert(
                            "اسم".to_string(),
                        ...
                        let layers_data: Vec<SharedValue> = layers
                            .iter()
                            .map(|l| {
                                let mut layer_dict = HashMap::new();
                                layer_dict.insert(
                                    "نوع_طبقة".to_string(),
                                    Rc::new(RefCell::new(Value::String(l.layer_type.clone()))),
                                );
                                layer_dict.insert(
                                    "مدخل".to_string(),
                                    Rc::new(RefCell::new(Value::Number(l.input_size as f64))),
                                );
                                layer_dict.insert(
                                    "مخرج".to_string(),
                                    Rc::new(RefCell::new(Value::Number(l.output_size as f64))),
                                );
                        ...
                        data.insert(
                            "طبقات".to_string(),
                            Rc::new(RefCell::new(Value::List(layers_data))),
                        );
                        data
                    }
                ...
                result.insert(
                    "بيانات".to_string(),
                    Rc::new(RefCell::new(Value::String(json_result))),
                );
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                result.insert(
                    "رسالة".to_string(),
                    Rc::new(RefCell::new(Value::String("تم التحميل بنجاح".to_string()))),
                );
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                // تحويل إلى نص
                let weights_str: Vec<String> =
                    weights.iter().map(|w| format!("{:.6}", w)).collect();
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                let input_size = a[0].borrow().to_number()? as usize;
                let output_size = a[1].borrow().to_number()? as usize;
                let num_heads = if a.len() > 2 {
                    a[2].borrow().to_number()? as usize
                } else {
                    4
                };
                let d_model = if a.len() > 3 {
                    a[3].borrow().to_number()? as usize
                } else {
                    128
                };
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                loader.insert(
                    "حجم_دفعة".to_string(),
                    Rc::new(RefCell::new(Value::Number(batch_size as f64))),
                );
                loader.insert(
                    "خلط".to_string(),
                    Rc::new(RefCell::new(Value::Boolean(shuffle))),
                );
                loader.insert(
                    "فهرس".to_string(),
                    Rc::new(RefCell::new(Value::Number(0.0))),
                ...
                match data {
                    Value::List(l) => {
                        ...
                        loader.insert(
                            "عدد".to_string(),
                            Rc::new(RefCell::new(Value::Number(loader.len() as f64))),
                        );
                    }
            ...
            func: |a| {
                ...
                match &mut *loader.borrow_mut() {
                    Value::Dictionary(dict) => {
                        ...
                        Err("بيانات المحمل غير صالحة".to_string())
                    }
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                result.insert(
                    "فاصل".to_string(),
                    Rc::new(RefCell::new(Value::String(delimiter))),
                );
                result.insert(
                    "صفوف".to_string(),
                    Rc::new(RefCell::new(Value::Number(0.0))),
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                let matrix: Vec<SharedValue> = data
                    .iter()
                    .map(|row| match &*row.borrow() {
                        Value::List(items) => {
                            let nums: Vec<SharedValue> = items
                                .iter()
                                .map(|v| {
                                    Rc::new(RefCell::new(Value::Number(
                                        v.borrow().to_number().unwrap_or(0.0),
                                    )))
                                })
                                .collect();
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                // تطبيق Dropout
                let result: Vec<SharedValue> = values
                    .iter()
                    .map(|&v| {
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                output.insert(
                    "قيم".to_string(),
                    Rc::new(RefCell::new(Value::List(result))),
                );
                output.insert(
                    "متوسط".to_string(),
                    Rc::new(RefCell::new(Value::Number(mean))),
                );
                output.insert(
                    "تباين".to_string(),
                    Rc::new(RefCell::new(Value::Number(variance))),
                );
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                result.insert(
                    "عقوبة".to_string(),
                    Rc::new(RefCell::new(Value::Number(penalty))),
                );
                result.insert(
                    "أوزان".to_string(),
                    Rc::new(RefCell::new(Value::List(updated))),
                );
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                let clipped: Vec<SharedValue> = gradients
                    .iter()
                ...
                result.insert(
                    "تدرجات".to_string(),
                    Rc::new(RefCell::new(Value::List(clipped))),
                );
                result.insert(
                    "معيار_أصلي".to_string(),
                    Rc::new(RefCell::new(Value::Number(norm))),
                );
                result.insert(
                    "معيار_جديد".to_string(),
                    Rc::new(RefCell::new(Value::Number(norm * scale))),
                );
...
/// عداد عام للمعرفات - آمن للخيوط باستخدام AtomicUsize
/// هذا يحل محل static mut unsafe السابق
static AUTOGRAD_COUNTER: AtomicUsize = AtomicUsize::new(0);

/// الحصول على معرف فريد جديد للـ autograd
/// هذه الدالة آمنة تماماً ولا تحتاج unsafe
fn next_autograd_id() -> usize {
    AUTOGRAD_COUNTER.fetch_add(1, Ordering::SeqCst)
}

/// إعادة تعيين العداد (للاختبارات فقط)
#[allow(dead_code)]
fn reset_autograd_counter() {
    AUTOGRAD_COUNTER.store(0, Ordering::SeqCst);
}

/// الحصول على القيمة الحالية للعداد
#[allow(dead_code)]
fn current_autograd_counter() -> usize {
    AUTOGRAD_COUNTER.load(Ordering::SeqCst)
}
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                let requires_grad = if a.len() > 1 {
                    a[1].borrow().is_truthy()
                } else {
                    true
                };
            ...
            func: |a| {
                let (id1, data1, shape1, req1) = match &*a[0].borrow() {
                    Value::AutoTensor {
                        ...
                        requires_grad,
                        ..
                    } => (*id, data.clone(), shape.clone(), *requires_grad),
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                let result_data: Vec<f64> =
                    data1.iter().map(|x| 1.0 / (1.0 + (-x).exp())).collect();
                let id = next_autograd_id();
                    ...
                    results.push(Rc::new(RefCell::new(Value::Dictionary({
                        ...
                        dict.insert(
                            ...
                            Rc::new(RefCell::new(Value::List(
                                computed_grad
                                    .iter()
                                    .map(|g| Rc::new(RefCell::new(Value::Number(*g))))
                                    .collect(),
                            ))),
                        );
                        dict
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                Ok(Rc::new(RefCell::new(Value::Dictionary({
                    ...
                    dict.insert(
                        "خسارة".to_string(),
                        Rc::new(RefCell::new(Value::Number(loss_data[0]))),
                    );
                    dict.insert(
                        "تدرجات".to_string(),
                        Rc::new(RefCell::new(Value::List(results))),
                    );
                    dict
                }))))
            },
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                Ok(Rc::new(RefCell::new(Value::Dictionary({
                    ...
                    dict.insert(
                        "خسارة".to_string(),
                        Rc::new(RefCell::new(Value::Number(mse))),
                    );
                    dict.insert(
                        "تدرجات".to_string(),
                        Rc::new(RefCell::new(Value::List(
                    ...
                    dict.insert(
                        ...
                        Rc::new(RefCell::new(Value::List(
                            updated
                                .iter()
                                .map(|u| Rc::new(RefCell::new(Value::Number(*u))))
                                .collect(),
                        ))),
                    );
                    dict
        ...
        Value::NativeFunction {
            ...
            func: |a| match &*a[0].borrow() {
                Value::AutoTensor {
                    ...
                    info.insert(
                        "حجم".to_string(),
                        Rc::new(RefCell::new(Value::Number(data.len() as f64))),
                    );
                    info.insert(
                        "أبعاد".to_string(),
                        Rc::new(RefCell::new(Value::List(
                    ...
                    info.insert(
                        "يتطلب_تدرج".to_string(),
                        Rc::new(RefCell::new(Value::Boolean(*requires_grad))),
                    );

                    if let Some(op_name) = op {
                        info.insert(
                            "عملية".to_string(),
                            Rc::new(RefCell::new(Value::String(op_name.clone()))),
                        );
                    }

                    info.insert(
                        "عدد_الآباء".to_string(),
                        Rc::new(RefCell::new(Value::Number(parents.len() as f64))),
                    );
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                result.insert(
                    "وزن".to_string(),
                    Rc::new(RefCell::new(Value::Number(weight))),
                );
                result.insert(
                    "تحيز".to_string(),
                    Rc::new(RefCell::new(Value::Number(bias))),
                );
                result.insert(
                    "خسائر".to_string(),
                    Rc::new(RefCell::new(Value::List(
                        losses
                            .iter()
                            .map(|l| Rc::new(RefCell::new(Value::Number(*l))))
                            .collect(),
                    ))),
                );
                result.insert(
                    "خسارة_أخيرة".to_string(),
                    Rc::new(RefCell::new(Value::Number(*losses.last().unwrap_or(&0.0)))),
                );
            ...
            func: |a| {
                ...
                for _epoch in 0..epochs {
                    ...
                    for (input, target) in inputs.iter().zip(targets.iter()) {
                        ...
                        let output_delta: Vec<f64> = output_activation
                            .iter()
                            .zip(target.iter())
                            .zip(output_z.iter())
                ...
                result.insert(
                    "طبقات".to_string(),
                    Rc::new(RefCell::new(Value::Number(layer_sizes.len() as f64))),
                );
                result.insert(
                    "خطوات".to_string(),
                    Rc::new(RefCell::new(Value::Number(epochs as f64))),
                );
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                // التنبؤ
                let predictions: Vec<SharedValue> = inputs
                    .iter()
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                let grad_numerical = match func_name.as_str() {
                    ...
                    "تانه" => (x.tanh() - (x - epsilon).tanh()) / epsilon,
                    "مربع" => ((x + epsilon).powi(2) - (x - epsilon).powi(2)) / (2.0 * epsilon),
                ...
                result.insert(
                    "ناجح".to_string(),
                    Rc::new(RefCell::new(Value::Boolean(passed))),
                );
                result.insert(
                    "تدرج_عددي".to_string(),
                    Rc::new(RefCell::new(Value::Number(grad_numerical))),
                );
                result.insert(
                    "تدرج_تحليلي".to_string(),
                    Rc::new(RefCell::new(Value::Number(grad_analytical))),
                );
                result.insert(
                    "فرق".to_string(),
                    Rc::new(RefCell::new(Value::Number(diff))),
                );
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |_a| {
                ...
                result.insert(
                    "النوع".to_string(),
                    Rc::new(RefCell::new(Value::String("CPU+Parallel".to_string()))),
                );
                result.insert(
                    "الأنوية".to_string(),
                    Rc::new(RefCell::new(Value::Number(num_cpus::get() as f64))),
                );
                Ok(Rc::new(RefCell::new(Value::Dictionary(result))))
            },
        },
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                if k1 != k2 {
                    return Err(format!("أبعاد غير متوافقة: {}×{} و {}×{}", m, k1, k2, n));
                }
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                output.insert(
                    "وقت_مللي".to_string(),
                    Rc::new(RefCell::new(Value::Number(elapsed as f64))),
                );
                output.insert(
                    "حجم".to_string(),
                ...
                output.insert(
                    "جيجا_فلوب".to_string(),
                    Rc::new(RefCell::new(Value::Number(gflops))),
                );
                output.insert(
                    "عمليات".to_string(),
                    Rc::new(RefCell::new(Value::Number(ops))),
                );
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                output.insert(
                    "وقت_مللي".to_string(),
                    Rc::new(RefCell::new(Value::Number(cpu_time as f64))),
                );
                output.insert(
                    "حجم".to_string(),
                    Rc::new(RefCell::new(Value::Number(size as f64))),
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                onnx_data.insert(
                    ...
                    Rc::new(RefCell::new(Value::String("7".to_string()))),
                );
                onnx_data.insert(
                    "producer_name".to_string(),
                    Rc::new(RefCell::new(Value::String("AlMarjaa".to_string()))),
                );
                onnx_data.insert(
                    "producer_version".to_string(),
                    Rc::new(RefCell::new(Value::String("2.0.0".to_string()))),
                );
                onnx_data.insert(
                ...
                for (i, layer) in layers.iter().enumerate() {
                    ...
                    node.insert(
                        "name".to_string(),
                        Rc::new(RefCell::new(Value::String(format!("layer_{}", i)))),
                    );
                    node.insert(
                        "op_type".to_string(),
                        Rc::new(RefCell::new(Value::String(layer.layer_type.clone()))),
                    );
                    node.insert(
                ...
                onnx_data.insert(
                    ...
                    Rc::new(RefCell::new(Value::List(graph_nodes))),
                );
                ...
                result.insert(
                    "صيغة".to_string(),
                    Rc::new(RefCell::new(Value::String("ONNX".to_string()))),
                );
                result.insert(
                    "بيانات".to_string(),
                    Rc::new(RefCell::new(Value::String(json_result))),
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                torch_data.insert(
                    "format".to_string(),
                    Rc::new(RefCell::new(Value::String("pytorch".to_string()))),
                );
                torch_data.insert(
                    "model_name".to_string(),
                    Rc::new(RefCell::new(Value::String(name))),
                ...
                for (i, layer) in layers.iter().enumerate() {
                    ...
                    layer_dict.insert(
                        "key".to_string(),
                        Rc::new(RefCell::new(Value::String(format!("layers.{}.weight", i)))),
                    );
                    layer_dict.insert(
                        "shape".to_string(),
                        Rc::new(RefCell::new(Value::List(vec![
                ...
                torch_data.insert(
                    ...
                    Rc::new(RefCell::new(Value::List(state_dict))),
                );
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                tf_data.insert(
                    "saved_model_schema_version".to_string(),
                    Rc::new(RefCell::new(Value::String("1".to_string()))),
                );
                tf_data.insert(
                    "model_name".to_string(),
                    Rc::new(RefCell::new(Value::String(name))),
                ...
                sig_def.insert(
                    ...
                    Rc::new(RefCell::new(Value::String(
                        "tensorflow/serving/predict".to_string(),
                    ))),
                );
                ...
                for (i, layer) in layers.iter().enumerate() {
                    if i == 0 {
                        ...
                        input_node.insert(
                            "name".to_string(),
                            Rc::new(RefCell::new(Value::String("input".to_string()))),
                        );
                        input_node.insert(
                            "dtype".to_string(),
                            Rc::new(RefCell::new(Value::String("DT_FLOAT".to_string()))),
                        );
                        input_node.insert(
                            "shape".to_string(),
                            Rc::new(RefCell::new(Value::List(vec![
                ...
                sig_def.insert(
                    "inputs".to_string(),
                    Rc::new(RefCell::new(Value::List(inputs))),
                );
                signatures.push(Rc::new(RefCell::new(Value::Dictionary(sig_def))));
                tf_data.insert(
                    "signatures".to_string(),
                    Rc::new(RefCell::new(Value::List(signatures))),
                );
                ...
                result.insert(
                    "صيغة".to_string(),
                    Rc::new(RefCell::new(Value::String("TensorFlow".to_string()))),
                );
                result.insert(
                    "بيانات".to_string(),
                    Rc::new(RefCell::new(Value::String(json_result))),
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                let json_result = match &*model.borrow() {
                    ...
                    Value::Dictionary(d) => {
                        إلى_جسون_داخلي(&Value::Dictionary(d.clone()))
                    }
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                checkpoint.insert(
                    "epoch".to_string(),
                    Rc::new(RefCell::new(Value::Number(epoch as f64))),
                );
                checkpoint.insert(
                    "model_name".to_string(),
                    Rc::new(RefCell::new(Value::String(name))),
                ...
                let layers_data: Vec<SharedValue> = layers
                    ...
                    .map(|(i, l)| {
                        ...
                        layer_dict.insert(
                            "index".to_string(),
                            Rc::new(RefCell::new(Value::Number(i as f64))),
                        );
                        layer_dict.insert(
                            "type".to_string(),
                            Rc::new(RefCell::new(Value::String(l.layer_type.clone()))),
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                result.insert(
                    "model".to_string(),
                    Rc::new(RefCell::new(Value::NeuralNetwork {
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                // محاكاة ضغط النموذج
                let compressed_layers: Vec<_> = layers
                    .iter()
                    .map(|l| {
                ...
                let original_params: usize =
                    layers.iter().map(|l| l.input_size * l.output_size).sum();
                let compressed_params: usize = compressed_layers
                    .iter()
                ...
                result.insert(
                    ...
                    Rc::new(RefCell::new(Value::Number(original_params as f64))),
                );
                result.insert(
                    "compressed_params".to_string(),
                    Rc::new(RefCell::new(Value::Number(compressed_params as f64))),
                );
                result.insert(
                    "ratio".to_string(),
                    Rc::new(RefCell::new(Value::Number(ratio))),
                );
                result.insert(
                    "compression_percent".to_string(),
                    Rc::new(RefCell::new(Value::Number(
    ...
    env.define(
        ...
        Value::NativeFunction {
            ...
            func: |a| {
                ...
                result.insert(
                    "bits".to_string(),
                    Rc::new(RefCell::new(Value::Number(bits as f64))),
                );
                result.insert(
                    "original_size_bytes".to_string(),
                    Rc::new(RefCell::new(Value::Number(original_size))),
                );
                result.insert(
                    "quantized_size_bytes".to_string(),
                    Rc::new(RefCell::new(Value::Number(quantized_size))),
                );
                result.insert(
