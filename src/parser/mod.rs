...
/// نوع معامل الدالة مع التعليقات التوضيحية
pub type ParameterWithType = (String, Option<Expr>, Option<TypeAnnotation>);
...
impl Parser {
    ...
    fn parse_identifier(&mut self) -> Result<String, ParseError> {
        match &self.current_token().token_type {
            ...
            // معاملة بعض الكلمات المفتاحية كمعرفات (للذكاء الاصطناعي والدوال المدمجة)
            TokenType::Model
            | TokenType::Train
            | TokenType::Predict
            | TokenType::Infer
        ...
        match &token.token_type {
            TokenType::Identifier(name) => {
                ...
                match name.as_str() {
                    ...
                    "منطقي" | "bool" | "boolean" => {
                        Ok(TypeAnnotation::Simple("منطقي".to_string()))
                    }
                    "قائمة" | "list" => {
            ...
            TokenType::LParen => {
                ...
                if self.match_token(&[TokenType::Arrow]) {
                    ...
                    Ok(TypeAnnotation::Function(
                        params,
                        Box::new(TypeAnnotation::Simple("لا_شيء".to_string())),
                    ))
                }
            }
            _ => Err(ParseError {
    ...
    fn parse_parameters_with_types(&mut self) -> Result<Vec<ParameterWithType>, ParseError> {
        let mut params = Vec::new();
    ...
    fn parse_primary(&mut self) -> Result<Expr, ParseError> {
        ...
        match &token.token_type {
            ...
            // معاملة الكلمات المفتاحية للذكاء الاصطناعي كمعرفات
            TokenType::Sigmoid | TokenType::Relu | TokenType::Softmax |
            TokenType::Train | TokenType::Predict | TokenType::Infer |
            TokenType::Load | TokenType::Save | TokenType::Model |
            TokenType::Tensor | TokenType::Layer | TokenType::Dense |
            TokenType::Conv | TokenType::Pool | TokenType::Batch |
            TokenType::Optimizer | TokenType::Loss | TokenType::Shape |
            TokenType::Flatten | TokenType::Reshape | TokenType::Dropout |
            TokenType::Activation | TokenType::Output | TokenType::Normalize |
            // معاملة كلمات واجهة المستخدم كمعرفات
            TokenType::Button | TokenType::Input | TokenType::Select |
            TokenType::Table | TokenType::Form | TokenType::InputField |
            TokenType::Chart | TokenType::LineChart | TokenType::BarChart |
            TokenType::PieChart => {
                let name = token.token_type.to_string();
                self.advance();
                Ok(Expr::Identifier(name))
